<template>
  <div class="p-6">
    <!-- Header -->
    <div class="mb-6">
      <h1 class="text-2xl font-bold text-gray-900 mb-2">Gerenciamento de Fornecedores</h1>
      <p class="text-gray-600">Gerencie os fornecedores de produtos da sua empresa</p>
    </div>
    ...
    <div class="bg-white rounded-lg shadow">
      ...
      <div v-else-if="fornecedores.length > 0" class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="forn in fornecedores" :key="forn.id" class="hover:bg-gray-50">
              <td class="px-6 py-4">
                <div class="text-sm font-medium text-gray-900">{{ forn.nome }}</div>
                <div v-if="forn.razaoSocial" class="text-sm text-gray-500">
                  {{ forn.razaoSocial }}
                </div>
              </td>
              <td class="px-6 py-4 text-sm text-gray-500">
                {{ forn.cnpjFormatado || formatarCnpj(forn.cnpj) }}
              </td>
              <td class="px-6 py-4">
                <div class="text-sm text-gray-900">{{ formatarTelefone(forn.telefone) }}</div>
                <div class="text-sm text-gray-500">{{ forn.email }}</div>
              </td>
              <td class="px-6 py-4 text-sm text-gray-500">
              ...
              <td class="px-6 py-4">
                <span
                  :class="[
                    ...
                    forn.ativo ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800',
                  ]"
                >
              ...
              <td class="px-6 py-4 text-sm font-medium">
                <div class="flex gap-2">
                  ...
                  <button
                    v-if="forn.ativo"
                    @click="inativarFornecedor(forn.id)"
                  ...
                  <button
                    v-else
                    @click="ativarFornecedor(forn.id)"
      ...
      <div v-else class="text-center py-12">
        ...
        <button
          ...
          class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700"
        >
          <i class="fas fa-plus mr-2"></i>Criar Fornecedor
        </button>
      </div>

      <!-- Paginação -->
      <div
        v-if="fornecedores.length > 0 && totalPaginas > 1"
      ...
      >
        ...
        <div>
          <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px">
            <button
              @click="paginaAtual > 1 && (paginaAtual--, listarFornecedores())"
            ...
            <button
              @click="paginaAtual < totalPaginas && (paginaAtual++, listarFornecedores())"
    ...
    <div
      v-if="fornecedorParaExcluir"
      class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50"
    >
      <div class="bg-white rounded-lg shadow-xl p-6 w-full max-w-md mx-4">
        <h3 class="text-lg font-medium mb-4">Confirmar Exclusão</h3>
        <p class="text-sm text-gray-500 mb-4">
          Tem certeza que deseja excluir <strong>{{ fornecedorParaExcluir.nome }}</strong
          >?
        </p>
